...
public class ProductResource {
    ...
    public Uni<Response> getAllProducts(
        ...
        return productService.findPaginatedSorted(pageIndex, pageSize, sort)
    ...
    public Uni<Response> createProduct(Product product) {
        log.info("Creating new product: {}, code: {}", product.getName(), product.getCode());
        ...
        return productService.existsByCode(product.getCode())
                .onItem().transformToUni(exists -> {
                    if (exists) {
                        log.warn("Product with code {} already exists", product.getCode());
                        return Uni.createFrom().item(Response.status(Response.Status.CONFLICT)
                                .entity("Product with code " + product.getCode() + " already exists")
                                .build());
                    } else {
