...
public class RegionResource {
    ...
    public Uni<Response> createRegion(Region region) {
        log.info("Creating new region: {}, country: {}", region.name(), region.country());
        ...
        if (region.country() == null || region.country().trim().isEmpty()) {
            return Uni.createFrom().item(Response.status(Response.Status.BAD_REQUEST)
                    .entity("Country is required")
                    .build());
        }

        return regionService.existsByNameAndCountry(region.name(), region.country())
                .onItem().transformToUni(exists -> {
                    if (exists) {
                        log.warn("Region with name {} and country {} already exists", region.name(), region.country());
                        return Uni.createFrom().item(Response.status(Response.Status.CONFLICT)
                                .entity("Region with name " + region.name() + " and country " + region.country() + " already exists")
                                .build());
                    } else {
                        return regionService.save(region)
                                .onItem().transform(savedRegion -> {
                                    log.info("Successfully created region with id: {}", savedRegion.id());
