...
public class CustomerResource {
    ...
    public Uni<Response> createCustomer(Customer customer) {
        log.info("Creating new customer: {}", customer != null ? customer.name() : "null");

        return Uni.createFrom().item(customer)
                .onItem().transform(c -> c == null || c.name() == null || c.name().trim().isEmpty())
                .onItem().transform(isInvalid -> {
                    if (isInvalid) {
                        return Response.status(Response.Status.BAD_REQUEST)
                ...
                .onItem().ifNull().switchTo(() ->
                        {
                            ...
                            return Uni.createFrom().item(customer.name().length() > 200)
                                    .onItem().transform(isInvalid -> {
                                        if (isInvalid) {
                                            return Response.status(Response.Status.BAD_REQUEST)
                ...
                .onFailure().recoverWithItem(throwable -> {
                    log.error("Error creating customer: {}", customer != null ? customer.name() : "null", throwable);
                    return Response.status(Response.Status.BAD_REQUEST)
    ...
    public Uni<Response> updateCustomer(@PathParam("id") Long id, Customer customer) {
        ...
        return Uni.createFrom().item(id)
                ...
                .onItem().ifNotNull().transform(response -> response)
                .onItem().ifNull().switchTo(() ->
                        Uni.createFrom().item(customer.name().length() > 200)
                                .onItem().transform(isInvalid -> {
                                    if (isInvalid) {
                                        return Response.status(Response.Status.BAD_REQUEST)
                                                .entity("Customer name must not exceed 200 characters")
