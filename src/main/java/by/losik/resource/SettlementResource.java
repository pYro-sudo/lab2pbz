...
public class SettlementResource {
    ...
    public Uni<Response> getAllSettlements(
        ...
        return settlementService.findPaginatedSorted(pageIndex, pageSize, sort)
    ...
    public Uni<Response> createSettlement(Settlement settlement) {
        ...
        if (settlement.getName() == null || settlement.getName().trim().isEmpty()) {
            return Uni.createFrom().item(Response.status(Response.Status.BAD_REQUEST)
                    .entity("Settlement name is required")
                    .build());
        }

        if (settlement.getRegion() == null || settlement.getRegion().getId() == null) {
            return Uni.createFrom().item(Response.status(Response.Status.BAD_REQUEST)
                    .entity("Region ID is required")
                    .build());
        }

        return settlementService.existsByNameAndRegionId(settlement.getName(), settlement.getRegion().getId())
                .onItem().transformToUni(exists -> {
                    if (exists) {
                        log.warn("Settlement with name {} and region id {} already exists",
                                settlement.getName(), settlement.getRegion().getId());
                        return Uni.createFrom().item(Response.status(Response.Status.CONFLICT)
                                .entity("Settlement with name " + settlement.getName() + " and region id " +
                                        settlement.getRegion().getId() + " already exists")
                                .build());
                    } else {
                        return settlementService.save(settlement)
                                .onItem().transform(savedSettlement -> {
                                    log.info("Successfully created settlement with id: {}", savedSettlement.getId());
